...
const RentalModal = ({ isOpen, onClose, item, selectedDate, selectedTimeSlot, totalPrice, onTotalPriceChange }) => {
  ...
  const handlePayment = async (rentalData) => {
    try {
      ...
      setError(error.response?.data?.message || 'Error al procesar el pago. Intente nuevamente.');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rentalData = {
        ...
        ownerId: item.userId,
        startDate: item.priceType === 'daily' 
          ? selectedDate.start.toISOString().split('T')[0] 
          : selectedDate.toISOString().split('T')[0],
        endDate: item.priceType === 'daily' 
      ...
      setError(error.response?.data?.message || 'Error al crear la reserva. Por favor, intente nuevamente.');
      setLoading(false);
    }
  };
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-medium text-gray-900">Detalles del Artículo</h3>
              <p className="text-gray-600">{item.title}</p>
              <p className="text-gray-600">
                {item.priceType === 'hourly' ? 'Alquiler por hora' : 'Alquiler por día'}
              </p>
            </div>

            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-medium text-gray-900">Detalles del Alquiler</h3>
              <p className="text-gray-600">
                Fecha: {selectedDate.start ? 
              ...
              <div className="mt-2 space-y-2">
                ...
                <div className="flex justify-between">
                  <div className="flex items-center">
                    <span className="text-gray-600">Depósito de garantía ({DEPOSIT_PERCENTAGE}%):</span>
                    <button 
                ...
                {showDepositInfo && (
                  <div className="bg-blue-50 p-3 rounded-md text-sm text-blue-700 mt-2">
                    <p className="mb-1">El depósito de garantía se devuelve al finalizar el alquiler si el artículo se devuelve en buenas condiciones.</p>
                    <p>La plataforma retiene una comisión del {PLATFORM_FEE_PERCENTAGE}% (${platformFee.toLocaleString()}) por el servicio.</p>
                  </div>
                )}
                ...
                <p className="text-sm text-gray-500 mt-1">
                  El resto se pagará directamente al propietario al momento de la entrega.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {contactWarning && (
                <p className="text-sm text-yellow-600">
                  Por favor, no incluya información de contacto en las notas. Puede comunicarse con el propietario a través del chat una vez confirmada la reserva.
                </p>
              )}
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-4">
              ...
              <button
                ...
                className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 ${(loading || paymentProcessing || contactWarning) ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                {paymentProcessing ? (
