        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-semibold text-gray-600 mb-2">Balance Disponible</h3>
              <p className="text-3xl font-bold text-[#009688]">{formatCurrency(balance.available)}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-semibold text-gray-600 mb-2">Pendiente</h3>
              <p className="text-3xl font-bold text-[#FFC107]">{formatCurrency(balance.pending)}</p>
            </div>
            <div className="bg-white rounded-2xl shadow-lg p-6">
              <h3 className="text-lg font-semibold text-gray-600 mb-2">Balance Total</h3>
              <p className="text-3xl font-bold text-gray-800">{formatCurrency(balance.total)}</p>
            </div>
          </div>
            ...
            <div className="p-6">
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-4">Próximos Alquileres</h3>
                            ...
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-sm text-gray-500">
                                  {rental.ownerId === auth.currentUser.uid ? 'Como Propietario' : 'Como Inquilino'}
                                </p>
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeClass(rental.status)}`}>
                                {rental.status}
                              </span>
                            </div>
                  ...
                  {rentals.asOwner.map(rental => (
                    <div key={rental.id} className="bg-gray-50 rounded-lg p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium">{rental.itemTitle}</p>
                          <p className="text-sm text-gray-500">Alquilado por: {rental.renterName}</p>
                        </div>
                        <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeClass(rental.status)}`}>
                          {rental.status}
                        </span>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-4">
                        <div>
                          <p className="text-sm text-gray-600">
                            Fecha: {formatDate(rental.startDate)} - {formatDate(rental.endDate)}
                          </p>
                          <p className="text-sm text-gray-600">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-500">
                            Fee: {formatCurrency(rental.totalAmount * 0.10)}
                          </p>
                        </div>
                      </div>
                    </div>
                  ...
                  {rentals.asRenter.map(rental => (
                    <div key={rental.id} className="bg-gray-50 rounded-lg p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-medium">{rental.itemTitle}</p>
                          <p className="text-sm text-gray-500">Propietario: {rental.ownerName}</p>
                        </div>
                        <span className={`px-2 py-1 rounded-full text-xs ${getStatusBadgeClass(rental.status)}`}>
                          {rental.status}
                        </span>
