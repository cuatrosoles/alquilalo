...
const createNewItem = async (req, res) => {
  try {
    ...
    const { categoryId, title, description, pricePerDay, depositAmount, location, latitude, longitude, availabilityStart, availabilityEnd } = req.body;
    const images = req.files ? await itemService.uploadImages(req.files) : []; // Subir imágenes si se proporcionan
    const item = await itemService.createItem(userId, categoryId, title, description, pricePerDay, depositAmount, location, latitude, longitude, availabilityStart, availabilityEnd, images);
...
const getAllItemsHandler = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: error.message || 'Error al obtener los artículos' });
  }
};
...
const updateExistingItem = async (req, res) => {
  try {
    ...
    if (!existingItem || existingItem.userId !== userId) {
      return res.status(403).json({ message: 'No estás autorizado para actualizar este artículo' });
    }

    const updates = req.body;
    const updatedImages = req.files ? await itemService.uploadImages(req.files) : [];
...
const deleteExistingItem = async (req, res) => {
  try {
    ...
    const existingItem = await itemService.getItemById(itemId);
    if (!existingItem || existingItem.userId !== userId) {
      return res.status(403).json({ message: 'No estás autorizado para eliminar este artículo' });
    }

    await itemService.deleteItem(itemId);
    res.json({ message: `Artículo con ID ${itemId} eliminado exitosamente` });
  } catch (error) {
    res.status(500).json({ message: error.message || 'Error al eliminar el artículo' });
  }
};
